...
def couchdb(db_name,URL, db_nameSecond, db_nameThird):
    ...
    while contador < 3 :
        ...
        urlFrancia='http://'+URL+':5984/'+db_name+'/_design/vistafinal/_view/vistafinal'
        urlRusia='http://'+URL+':5984/'+db_name+'/_design/partfinal/_view/partfinal'
        if db_name=='belgica':
            url=urlBelgica
        elif db_name=='francia':
            url=urlFrancia
        else:
            url=url
        ...
        for x in d['rows']:
            ...
            if lstHashtags["principal"] in texto or lstHashtags["secundario"] in texto or lstHashtags["alterno"] or lstHashtags[db_name] in texto:
                ...
                clasificador = textblob.TextBlob(tweetLimpio)
                if clasificador.sentiment.polarity > 0:
                    lstHoras[0]+=1
                elif clasificador.sentiment.polarity < 0:
                    lstHoras[1]+=1
                else: 
                    lstHoras[2]+=1
        contador= contador+1
    ...
    #----------------------------------------------------------------------
    explode=(0.05, 0, 0.1)#exposicion de uno de los datos segun donde se encuentra
    #tipo de grafico(datos,exposicion, titulos de los datos, nose,sombras true o false
    pie(lstHoras, explode=explode,labels=labels, autopct='%10.0f%%', shadow=True)
    legend()
    title('Evaluacion de Sentimientos', bbox={'facecolor':'0.8', 'pad':5})
